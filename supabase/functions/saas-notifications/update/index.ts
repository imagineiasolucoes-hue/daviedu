...
serve(async (req) => {
  ...
  try {
    const { id, title, content, image_url, external_link, target_tenant_id, target_role, is_global } = await req.json();
    ...
    if (profileError || !profileData || profileData.role !== 'super_admin') {
      console.error("Permission Error: User role not allowed to update notifications.", profileData?.role);
      return new Response(JSON.stringify({ error: 'Forbidden: Only Super Admins can update notifications' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (error) {
      console.error("Supabase Update Error (saas_notifications):", error);
    ...
    console.error("Edge Function CATCH block error (update-saas-notification):", errorMessage);
