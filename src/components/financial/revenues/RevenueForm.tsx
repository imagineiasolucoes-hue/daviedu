...
const RevenueForm: React.FC<RevenueFormProps> = ({
  ...
  const mutation = useMutation({
    mutationFn: async (values: z.infer<typeof revenueSchema>) => {
      ...
      if (isEditMode) {
        ...
        const { error } = await supabase.from("revenues").insert(submissionData);
        if (error) throw error;
      }
    },
    onSuccess: () => {
  ...
  const onSubmit = (values: z.infer<typeof revenueSchema>) => {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Ex: Mensalidade de Julho" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col">
                  <FormLabel>Vincular Aluno (Opcional)</FormLabel>
                  <Popover>
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
