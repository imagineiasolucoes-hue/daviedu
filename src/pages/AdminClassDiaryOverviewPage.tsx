...
const AdminClassDiaryOverviewPage: React.FC = () => {
  ...
  const handleDeleteEntry = async (entryId: string) => {
    ...
    const { error } = await supabase
      ...
      .delete()
      .eq('id', entryId)
      .eq('tenant_id', tenantId); // Garante que só pode excluir do próprio tenant
            ...
            <Accordion type="single" collapsible className="w-full">
              {entries.map(entry => (
                <AccordionItem key={entry.id} value={entry.id}>
                  <AccordionTrigger className="flex justify-between items-center p-4 hover:bg-muted/50">
                    <div className="flex flex-col items-start">
                      <span className="font-semibold text-base">
                        {entry.classes?.name || 'N/A'} - {entry.employees?.full_name || 'N/A'}
                      </span>
                    ...
                    <Button
                      ...
                      size="icon"
                      onClick={(e) => {
                        e.stopPropagation(); // Evita que o accordion se expanda ao clicar no botão
                  ...
                  <AccordionContent>
                    <div className="p-4 border-t">
                      <ClassDiaryEntryCard
                        ...
                        showActions={false} // Admins apenas visualizam, sem edição/exclusão aqui
                      />
                    </div>
                  </AccordionContent>
                </AccordionItem>
